...
| Features | 9 (deployed model) |
| Accuracy | **99.57%** |
| Precision | 0.991 |
| Recall | 0.991 |
| F1-Score | **0.991** |

> **Note:** Earlier model versions reported 93% accuracy with 17 features. The deployed model was retrained via the production pipeline (`ai_service/retrain_pipeline.py`) with real sensor data and an optimized 9-feature set.
...
| **Physics** | vpd (Vapor Pressure Deficit) |
| **Flags** | is_extreme_vpd, is_raining, is_high_wind |

See [ML Model Guide](../guides/ml_model_guide.md) for the complete decision engine, VPD science, training pipeline, and retraining workflow.
