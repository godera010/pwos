...
| **ML Model** | Random Forest (9 features) | `src/backend/models` | - |
...
The P-WOS system supports two distinct execution paths for local production. **Choose one:**

### Path A: Local Development & Testing (Simulation)
This mode runs entirely in software, artificially generating sensor data. Use this if you do not have physical hardware connected.
```batch
rem 1. First-time setup (creates .venv & installs dependencies)
setup.bat

rem 2. Launch Simulated System
start_simulation.bat
```

### Path B: Local Production (Physical ESP32 Hardware)
This mode connects your local software stack to the physical ESP32 sitting on your desk. 
> [!WARNING]
> Do NOT run `start_simulation.bat` if your physical ESP32 is powered on, as the conflicting data streams will corrupt the database.
```batch
rem 1. First-time setup (creates .venv & installs dependencies)
setup.bat

rem 2. Launch Hardware System
start_pwos.bat
```
...
| Feature | Description |
|---------|-------------|
| **9-Feature ML Model** | VPD, wind, rain, forecast integration |
