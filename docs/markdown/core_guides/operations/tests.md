...
> All 116 tests pass with `pytest src/backend/tests/ -v`. No skipped or xfail tests.

---

## Known Fixes & Notes

### Seasonal Mock Fix (May 2026)
Tests `test_vpd_delay_midday` (decision_logic.py) and `test_heatwave_prevention` (environmental.py) previously failed in October because the production code uses the real calendar month to calculate seasonal thresholds (summer/winter). The tests were updated to explicitly mock `datetime.now()` to return a dry-season October date, making the tests season-independent:

```python
# decision_logic.py — seasonal mock pattern
with patch('models.ml_predictor.datetime') as mock_dt:
    mock_dt.now.return_value = datetime(2026, 10, 15, 13, 0, 0)  # October (spring/dry)
    result = predictor.predict_next_watering(sensor_data)
    assert result['recommended_action'] == 'STALL'
```

### Pool Compatibility
All database tests use `unittest.mock.patch` on `psycopg2.connect`. The `PoolConnectionWrapper` in `database.py` automatically detects the mock and bypasses the pool, so all 10 `database_ops.py` tests remain fully functional without modifications.
