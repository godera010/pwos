...
let cachedBaseUrl = `http://${getDefaultIp()}:5000/api`;
...
export const updateApiBaseUrl = (customIp: string) => {
  if (customIp && customIp.trim() !== '') {
    ...
    cachedBaseUrl = `http://${getDefaultIp()}:5000/api`;
  }
};
...
export const api = {
    ...
    getMLDecisions: async (limit = 100): Promise<MLDecision[]> => {
        const res = await fetch(`${getApiBaseUrl()}/ml-decisions?limit=${limit}`);
        return handleResponse(res);
    },
    getModelVersions: async (): Promise<ModelVersion[]> => {
        const res = await fetch(`${getApiBaseUrl()}/model-versions`);
        return handleResponse(res);
    },
    getEfficiencySummary: async (hours = 168): Promise<EfficiencySummary> => {
        const res = await fetch(`${getApiBaseUrl()}/efficiency/summary?hours=${hours}`);
        return handleResponse(res);
    },
    ...
    getActiveCrop: async (): Promise<Crop> => {
        const res = await fetch(`${getApiBaseUrl()}/crops/active`);
        return handleResponse(res);
    },
    ...
    retrainModel: async (): Promise<{ status: string; message: string }> => {
        const res = await fetch(`${getApiBaseUrl()}/models/retrain`, {
            method: 'POST'
        });
        return handleResponse(res);
    }
