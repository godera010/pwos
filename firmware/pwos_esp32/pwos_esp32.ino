...
#define TOPIC_SYSTEM_HARDWARE "pwos/system/hardware"
#define TOPIC_SYSTEM_MODE     "pwos/system/mode"
#define TOPIC_DEVICE_STATUS   "pwos/device/status"  // Detailed heartbeats
...
String   currentMode      = "AUTO"; // Default mode
...
void connectMQTT() {
    ...
    bool connected;
    // Set LWT (Last Will and Testament) so the system knows if the ESP32 disconnects
    // mqttClient.connect(clientID, user, pass, willTopic, willQos, willRetain, willMessage)
    if (strlen(MQTT_USER) > 0) {
        connected = mqttClient.connect(DEVICE_ID, MQTT_USER, MQTT_PASS, TOPIC_SYSTEM_HARDWARE, 1, true, "OFFLINE");
    } else {
        connected = mqttClient.connect(DEVICE_ID, TOPIC_SYSTEM_HARDWARE, 1, true, "OFFLINE");
    }
    ...
    if (connected) {
        ...
        mqttClient.publish(TOPIC_SYSTEM_HARDWARE, "ONLINE", true);
        ...
        bool sub2 = mqttClient.subscribe(TOPIC_SYSTEM_MODE);
        Serial.printf("[MQTT] Subscribe to %s: %s\n", TOPIC_SYSTEM_MODE, sub2 ? "OK" : "FAILED");
